      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 overflow-hidden">
        <div class="flex flex-col md:flex-row md:items-end justify-between gap-6 overflow-hidden">
          ...
          <div class="md:w-1/2 space-y-3">
            <h1 class="text-4xl md:text-5xl font-black font-outfit text-[var(--accent-secondary)] leading-tight">
              <span class="text-[#C9A050]">BPGROUP</span> AOE Tournament
            </h1>

            <p class="text-[var(--text-secondary)] font-medium leading-relaxed">
          ...
          <div class="flex items-center gap-2 bg-[var(--bg-card)] border border-[var(--border-color)] px-4 py-2 rounded-full shadow-lg">
            <div class="w-2 h-2 rounded-full bg-[var(--text-secondary)] opacity-30"></div>
        ...
        {streams.map((stream) => {
          ...
          return (
            <div key={stream.id} className="flex flex-col bg-[var(--bg-card)] rounded-3xl border border-[var(--border-color)] overflow-hidden transition-all duration-300 hover:shadow-2xl hover:border-[#C9A050]/20 group shadow-[var(--card-shadow)]">
                ...
                <div className="flex items-center gap-3">
                  ...
                  {editingId === stream.id ? (
                    <div className="flex items-center gap-1.5">
                      ...
                      />
                      <button onClick={() => saveName(stream.id)} className="text-emerald-500 hover:text-emerald-400"><Check size={14} /></button>
                    ...
                    <div className="flex items-center gap-2 group/player">
                      ...
                      <button
                        ...
                        className="opacity-0 group-hover/player:opacity-100 p-1 text-[var(--text-secondary)] hover:text-[#C9A050] transition-opacity"
                      >
                        <Edit2 size={12} />
                      </button>
                    </div>
              ...
              {/* Card Footer */}
              <div className="p-4 flex items-center justify-between text-[var(--text-secondary)] group-hover:text-[#C9A050] transition-colors bg-[var(--bg-card-hover)]/20">
